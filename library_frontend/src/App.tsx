      ...
      {!listing ? <Loading /> : listing.items.length ? <div className="catalog-table-wrap"><table className="catalog-table">
        ...
        <tbody>{listing.items.map((item) => {
          ...
          return <Fragment key={item.title_key}>
            <tr>
              <td><strong>{item.display_title}</strong><code className="core">{item.title_key}</code>{item.author && <small>{item.author}</small>}{item.canonical_genre && <small className="catalog-genre">통합 장르 {item.canonical_genre}</small>}{item.genre_review_required && <small className="blocked">장르 검토 {item.genre_candidates.join(" · ")}</small>}{item.work_bucket_ids.length > 0 && <small>work {item.work_bucket_ids.join(", ")} · 판본 {item.variant_ids.length}</small>}</td>
              ...
              {(["series", "kakao", "novelpia"] as const).map((platform) => { const value = item.platforms[platform]; const tags = value.tags?.length ? value.tags.map((tag) => `#${tag}`).join(" ") : ""; return <td key={platform}><StatusBadge status={value.status} /><span>{catalogMetric(item, platform)}</span>{value.genre && <small className="catalog-genre">장르 {value.genre}</small>}{tags && <small className="catalog-tags">{tags}</small>}{value.remote_url && <a href={value.remote_url ?? undefined} target="_blank" rel="noreferrer">원문 ↗</a>}</td>; })}
            ...
            {isOpen && <tr className="catalog-detail-row"><td colSpan={6}><div className="catalog-detail-grid">
              ...
              <section><strong>플랫폼 수집 근거</strong>{(["series", "kakao", "novelpia"] as const).map((platform) => { const value = item.platforms[platform]; const tags = value.tags?.length ? value.tags.map((tag) => `#${tag}`).join(" ") : ""; return <div className="catalog-platform-detail" key={platform}><b>{platformLabels[platform]} · {statusLabel(value.status)}</b><small>{value.remote_title ?? "원문 제목 없음"}</small>{value.genre && <small className="catalog-genre">장르 {value.genre}</small>}{tags && <small className="catalog-tags">{tags}</small>}<small>마지막 시도 {value.last_attempt_at ? new Date(value.last_attempt_at).toLocaleString("ko-KR") : "없음"}</small>{value.genre_collected_at && <small>장르 확인 {new Date(value.genre_collected_at).toLocaleString("ko-KR")}</small>}{value.tags_collected_at && <small>태그 확인 {new Date(value.tags_collected_at).toLocaleString("ko-KR")}</small>}{value.error_message && <small className="blocked">{value.error_message}</small>}</div>; })}</section>
            </div></td></tr>}
          </Fragment>;
        })}</tbody>
