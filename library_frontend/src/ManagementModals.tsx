...
type HumanRelationshipVerdict = "same_work_distinct_variant" | "distinct_work";

export function RelationshipManager({ leftId, rightId, currentDecisionId, close, started }: { leftId: string; rightId: string; currentDecisionId?: number | null; close: () => void; started: (job: JobRecord) => void }) {
  const [verdict, setVerdict] = useState<HumanRelationshipVerdict | "">("");
  const [variantKind, setVariantKind] = useState("other");
  const [note, setNote] = useState("");
  const [plan, setPlan] = useState<RelationshipPlan>();
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const preview = async () => { if (!verdict) return; setBusy(true); setError(""); try { setPlan(await postJson<RelationshipPlan>("/api/management/relationships/preview", { left_file_id: leftId, right_file_id: rightId, verdict, variant_kind: verdict === "same_work_distinct_variant" ? variantKind : "other", note })); } catch (reason) { setError(reason instanceof Error ? reason.message : "계획을 만들지 못했습니다."); } finally { setBusy(false); } };
  const apply = async () => { if (!plan || !verdict) return; setBusy(true); setError(""); try { const job = await postJson<JobRecord>("/api/management/relationships/apply", { left_file_id: leftId, right_file_id: rightId, verdict, variant_kind: verdict === "same_work_distinct_variant" ? variantKind : "other", note, confirm_count: plan.item_count, confirm_plan_sha256: plan.plan_sha256 }); started(job); close(); } catch (reason) { setError(reason instanceof Error ? reason.message : "판정을 저장하지 못했습니다."); setBusy(false); } };
  const cancel = async () => { if (!currentDecisionId || !window.confirm("현재 사람 판정을 취소하고 두 파일을 다시 미배정 상태로 돌릴까요?")) return; setBusy(true); try { const job = await postJson<JobRecord>(`/api/management/relationships/${currentDecisionId}/cancel`, {}); started(job); close(); } catch (reason) { setError(reason instanceof Error ? reason.message : "판정을 취소하지 못했습니다."); setBusy(false); } };
  return <Modal close={close}><Top eyebrow="HUMAN RELATIONSHIP · 1.4.0" title="두 파일 관계 확정" close={close}/><p className="management-description">현재 fingerprint에 묶인 사람 판단을 저장합니다. 다른 변경이 실행 중이면 안전한 단일 대기열에 등록하고, 실행 차례에 현재 상태를 다시 확인합니다.</p>{error && <div className="inline-error">{error}</div>}<div className="management-form"><label>판정<select value={verdict} onChange={(event) => { setVerdict(event.target.value as HumanRelationshipVerdict | ""); setPlan(undefined); }}><option value="">판정을 선택하세요</option><option value="same_work_distinct_variant">같은 작품의 다른 판본·부속</option><option value="distinct_work">서로 다른 작품 (제목 동일·유사 포함)</option></select></label>{verdict === "same_work_distinct_variant" && <label>판본 종류<select value={variantKind} onChange={(event) => { setVariantKind(event.target.value); setPlan(undefined); }}><option value="other">기타/extra</option><option value="revision">개정판</option><option value="adult">성인판</option><option value="translation">번역판</option><option value="base">기본판</option></select></label>}<label className="management-wide">판단 메모<input value={note} onChange={(event) => { setNote(event.target.value); setPlan(undefined); }} placeholder="왜 이 관계로 판단했는지 선택 입력"/></label></div><div className="inline-notice"><span>내용이 같은 중복이라면 관계 판정으로 보존하지 않고 비교 화면에서 남길 파일을 확인한 뒤 반대쪽을 격리하세요.</span></div>{plan && <PlanCheck sha={plan.plan_sha256} blockers={plan.blocked_reasons}/>}<footer><button className="button secondary" disabled={busy || !verdict} onClick={preview}>{busy ? "확인 중…" : "계획 확인"}</button>{currentDecisionId && <button className="button danger" disabled={busy} onClick={cancel}>현재 판정 취소</button>}<button className="button primary" disabled={busy || !plan?.apply_available} onClick={apply}>{plan?.mode === "correction" ? "판정 정정 저장" : "판정 저장"}</button></footer></Modal>;
}
...
export function FolderQuarantineManager({ folderPath, close, started }: { folderPath: string; close: () => void; started: (job: JobRecord) => void }) {
  ...
  return <Modal close={close} danger><Top eyebrow="FOLDER QUARANTINE · 1.4.0" title="폴더 전체 사용자 승인 격리" close={close}/>
    ...
    {plan && <><div className="management-paths"><small>현재: {plan.source_path}</small><small>격리: {plan.destination_path}</small></div>
      ...
      {plan.representative_transitions.length > 0 && <div className="folder-related-keepers"><strong>격리 후 대표 도서 변경</strong>{plan.representative_transitions.map((transition) => {
        const replacement = transition.remaining_active_managed_files.find((file) => file.file_id === transition.replacement_file_id);
        return <span key={transition.variant_id}>판본 #{transition.variant_id} · {transition.representative_file_id} → {replacement?.canonical_path ?? (transition.blocker ? `차단: ${transition.blocker}` : "활성 관리 판본 종료")}</span>;
      })}</div>}
...
export function QuickTitleCorrectionManager({ fileId, close, started }: { fileId: string; close: () => void; started: (job: JobRecord) => void }) {
  ...
  return <Modal close={close}><Top eyebrow="QUICK TITLE CORRECTION · 1.3.7" title="파일 제목 빠른 교정" close={close}/>
