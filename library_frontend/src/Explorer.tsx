...
import { api } from "./api";
import { FileRelocateManager, ManagedFolderAdoptManager, ManagedFolderManager, ManagedFolderRelocateManager, PurgeManager, QuarantineManager, RelationshipManager, RestoreManager } from "./ManagementModals";
import type {
  ExplorerComparison,
  ExplorerFile,
  ExplorerFileDetail,
...
function Header({ title, description }: { title: string; description: string }) {
  return <header className="page-header">
    <div><span className="eyebrow">LIBRARY MANAGEMENT · 1.3.3</span><h1>{title}</h1><p>{description}</p></div>
...
function FileInspector({ fileId, close, compareWith, quarantine, organize }: { fileId: string; close: () => void; compareWith: (fileId: string) => void; quarantine: () => void; organize: (file: ExplorerFile) => void }) {
  ...
  return <Modal close={close} wide><div className="explorer-modal-top"><div><span className="eyebrow">FILE INSPECTOR</span><h2>{detail?.file.name ?? "파일 상세 확인"}</h2></div><button className="button secondary" onClick={close}>닫기</button></div>
    ...
    {!detail ? !error && <div className="loading"><span />상세 정보를 확인하고 있습니다.</div> : <>
      ...
      <footer className="explorer-action-footer"><span>파일 정리와 격리는 계획 SHA와 현재 identity를 다시 확인한 뒤 실행됩니다.</span>{detail.actions.title_correction && <NavLink className="button secondary" to={`/review/titles?search=${encodeURIComponent(detail.file.name)}`}>제목 교정에서 열기</NavLink>}<button className="button secondary" disabled={!detail.file.active || detail.file.source !== "house"} onClick={() => organize(detail.file)}>이름·위치 정리</button><button className="button danger" disabled={!detail.actions.quarantine} onClick={quarantine}>사용자 승인 격리</button></footer>
    </>}
  </Modal>;
}
...
function FolderInspector({ folder, close, createManaged, adopt, relocate }: { folder: ExplorerFolder; close: () => void; createManaged: () => void; adopt: () => void; relocate: () => void }) {
  ...
  return <Modal close={close} wide><div className="explorer-modal-top"><div><span className="eyebrow">FOLDER INVENTORY</span><h2>{folder.name}</h2></div><button className="button secondary" onClick={close}>닫기</button></div>{error && <div className="inline-error">{error}</div>}{!detail ? !error && <div className="loading"><span/>폴더 파일을 확인하고 있습니다.</div> : <><code className="explorer-path">{detail.path}</code>{detail.managed_folder && <div className="inline-notice"><span>관리 폴더 · 작품 #{detail.managed_folder.work_bucket_id} · {detail.managed_folder.role}</span></div>}<div className="explorer-folder-summary"><span>DB 등록 <strong>{detail.registered_count}</strong></span><span>미등록·부속 <strong>{detail.unregistered_count}</strong></span><span>크기 <strong>{formatBytes(detail.total_size)}</strong></span></div>{detail.truncated && <div className="explorer-warning">안전 한도에 따라 처음 5,000개만 표시합니다.</div>}<div className="explorer-entry-list">{detail.entries.map((entry) => <article key={entry.path} className={entry.registered ? "registered" : "unregistered"}><div><strong>{entry.name}</strong><small>{entry.relative_path}</small></div><span>{entry.registered ? "DB 등록" : "부속/미등록"}{entry.symlink ? " · 링크" : ""}</span><b>{formatBytes(entry.size)}</b></article>)}</div><footer className="explorer-action-footer"><span>{detail.managed_folder ? "도서와 부속 파일을 함께 보존하며 폴더를 정리합니다." : "현재 폴더를 작품 관계에 등록하거나 같은 위치에 새 관리 폴더를 만들 수 있습니다."}</span>{!detail.managed_folder && <button className="button primary" onClick={adopt}>현재 폴더 관리 등록</button>}<button className="button secondary" onClick={createManaged}>새 관리 폴더</button><button className="button secondary" disabled={!detail.actions.rename} onClick={relocate}>폴더 이름·위치 정리</button></footer></>}</Modal>;
}
