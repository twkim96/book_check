...
function queuedJobNotice(job: JobRecord, action?: string): string {
  const label = action ?? ({
    management_relationship: "관계 판정",
    management_relationship_cancel: "관계 판정 취소",
    management_quarantine: "격리",
    management_file_relocate: "파일 정리",
    title_requeue: "제목 교정",
    management_folder_create: "관리 폴더 생성",
    management_folder_adopt: "관리 폴더 등록",
    management_folder_relocate: "폴더 정리",
    management_folder_quarantine: "폴더 격리",
    management_restore: "복원",
    management_purge: "영구 삭제",
  } as Record<string, string>)[job.job_type] ?? "변경";
  if (job.state === "queued") {
...
function Header({ title, description }: { title: string; description: string }) {
  return <header className="page-header">
    <div><span className="eyebrow">LIBRARY MANAGEMENT · 1.3.9</span><h1>{title}</h1><p>{description}</p></div>
