...
import { api } from "./api";
import { PurgeManager, QuarantineManager, RelationshipManager, RestoreManager } from "./ManagementModals";
import type {
  ExplorerComparison,
  ExplorerFile,
  ExplorerFileDetail,
...
function Header({ title, description }: { title: string; description: string }) {
  return <header className="page-header">
    <div><span className="eyebrow">LIBRARY MANAGEMENT · 1.3.2</span><h1>{title}</h1><p>{description}</p></div>
...
function FileInspector({ fileId, close, compareWith, quarantine }: { fileId: string; close: () => void; compareWith: (fileId: string) => void; quarantine: () => void }) {
  ...
  return <Modal close={close} wide><div className="explorer-modal-top"><div><span className="eyebrow">FILE INSPECTOR</span><h2>{detail?.file.name ?? "파일 상세 확인"}</h2></div><button className="button secondary" onClick={close}>닫기</button></div>
    ...
    {!detail ? !error && <div className="loading"><span />상세 정보를 확인하고 있습니다.</div> : <>
      ...
      <footer className="explorer-action-footer"><span>격리는 계획 SHA와 파일 identity를 다시 확인한 뒤 실행됩니다. 임의 이동은 1.3.3 범위입니다.</span>{detail.actions.title_correction && <NavLink className="button secondary" to={`/review/titles?search=${encodeURIComponent(detail.file.name)}`}>제목 교정에서 열기</NavLink>}<button className="button danger" disabled={!detail.actions.quarantine} onClick={quarantine}>사용자 승인 격리</button></footer>
    </>}
  </Modal>;
}
...
function FileCatalog() {
  ...
  return <><Header title="파일 탐색기" description="파일 근거를 확인하고 두 파일 관계를 확정하거나 선택한 판본을 안전하게 격리합니다."/><CatalogTabs active="files"/>
    {error && <div className="inline-error">{error}</div>}
    {jobNotice && <div className="inline-notice"><span>격리 작업을 시작했습니다. 현재 화면에서 계속 작업할 수 있습니다.</span><NavLink to={`/jobs/${jobNotice.job_id}`}>작업 이력 열기</NavLink></div>}
