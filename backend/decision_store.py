...
def _assert_manifest_leaf_matches_evidence(
    directory_fd: int, leaf: str, evidence
) -> None:
    info = os.stat(leaf, dir_fd=directory_fd, follow_symlinks=False)
    if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
        raise RuntimeError("activation manifest pathname is not a single-link regular file")
    if hasattr(os, "getuid") and info.st_uid != os.getuid():
        raise RuntimeError("activation manifest pathname is not owned by the current user")
    identity = (
    ...
    if identity != expected:
        raise RuntimeError("activation manifest pathname was replaced")
...
def prepare_actual_run(
    ...
    try:
        ...
        manifest_payload = {"run_id": run_id, "files": records}
        with opened_directory_nofollow(manifest_dir) as manifest_dir_fd:
            directory_info = os.fstat(manifest_dir_fd)
            if hasattr(os, "getuid") and directory_info.st_uid != os.getuid():
                raise RuntimeError("activation manifest directory owner mismatch")
            if stat.S_IMODE(directory_info.st_mode) != 0o700:
                os.fchmod(manifest_dir_fd, 0o700)
                os.fsync(manifest_dir_fd)
            assert_private_directory_fd(manifest_dir_fd, mode=0o700)
            flags = (
            ...
            try:
                ...
                with os.fdopen(os.dup(manifest_fd), "w", encoding="utf-8") as manifest:
                ...
                manifest_created_evidence = inspect_open_regular_file_fd(manifest_fd)
                manifest_sha256 = manifest_created_evidence.sha256
                raw = bytearray()
                os.lseek(manifest_fd, 0, os.SEEK_SET)
                while len(raw) <= manifest_created_evidence.size:
                    chunk = os.read(manifest_fd, min(
                        1024 * 1024,
                        manifest_created_evidence.size + 1 - len(raw),
                    ))
                    if not chunk:
                        break
                    raw.extend(chunk)
                os.lseek(manifest_fd, 0, os.SEEK_SET)
                if len(raw) != manifest_created_evidence.size:
                    raise RuntimeError("activation manifest changed while validating payload")
                parsed_payload = json.loads(raw.decode("utf-8"))
                if parsed_payload != manifest_payload:
                    raise RuntimeError("activation manifest payload does not match approved inventory")
                _assert_manifest_leaf_matches_evidence(
                    manifest_dir_fd, manifest_leaf, manifest_created_evidence
                )
                ...
                try:
                    with transaction(conn):
                        ...
                        manifest_identity = (
                            manifest_created_evidence.dev,
                            manifest_created_evidence.ino,
                            manifest_created_evidence.ctime_ns,
                            manifest_created_evidence.size,
                            manifest_created_evidence.mtime_ns,
                        )
            ...
            if manifest_created and manifest_path is not None:
                ...
                with opened_directory_nofollow(manifest_path.parent) as manifest_dir_fd:
                    if manifest_created_evidence is None:
                        raise OSError(
                            "activation manifest evidence unavailable; refusing pathname cleanup"
                        )
                    unlink_owned_at(
                        manifest_dir_fd,
                        manifest_leaf,
                        expected=manifest_created_evidence,
                    )
        except FileNotFoundError:
            pass
        except (OSError, RuntimeError) as unlink_exc:
...
def close_interrupted_claimed_approved_run(
    conn: sqlite3.Connection,
    run_id: str,
    *,
    house_dir: os.PathLike | str,
    ...
    run, gate, approved, operation_count, group_count, other_count = load_evidence()
    if run is None:
        raise RuntimeError("interrupted claimed run is not approved")
    if run["house_root"] != expected_house or run["temp_root"] != expected_temp:
        raise RuntimeError("interrupted claimed run roots do not match this server")
    claim_id = run["activation_claim"]
    if not claim_id or run["activated_at"] is not None:
        raise RuntimeError("interrupted claimed run has no pending activation claim")
    if run["manifest_path"] is not None or run["manifest_sha256"] is not None:
        raise RuntimeError("interrupted claimed run has registered manifest evidence")
    if gate is None or gate["value"] != "0" or approved is not None:
        raise RuntimeError("interrupted claimed run gate evidence does not match")
    if operation_count or group_count or actual_run_mutation_started(conn, run_id):
        raise RuntimeError(
            "interrupted claimed run requires manual recovery: "
            f"operations={operation_count}, groups={group_count}"
        )
    if other_count:
        raise RuntimeError("another approved or active run blocks claimed cleanup")
    _verify_backup_evidence(run["backup_path"], run["backup_sha256"])
    ...
    with transaction(conn):
        ...
        if (
            current_run is None
            or current_run["activation_claim"] != claim_id
            or current_run["activated_at"] is not None
            or current_run["manifest_path"] is not None
            or current[1] is None
            or current[1]["value"] != "0"
            or current[2] is not None
            or current[3:] != (0, 0, 0)
            or actual_run_mutation_started(conn, run_id)
        ):
            raise RuntimeError("interrupted claimed run changed during cleanup")
        _verify_backup_evidence(
            current_run["backup_path"], current_run["backup_sha256"]
        )
        transition_actual_run(
            conn,
            run_id,
            "cancelled",
            error="library server restarted during actual run activation",
        )
        remaining = doctor_issues(conn)
        if remaining:
            raise RuntimeError(
                "Doctor failed after interrupted claimed cleanup: "
                f"{remaining[0]['kind']}"
            )
    ...
    return {
        ...
        "doctor_issue_count": 0,
        "state": "cancelled",
        "claimed_activation": True,
        "orphan_manifest_removed": manifest_removed,
    }
