...
### 1.4.23 Sheet 통합 조회수와 Chrome 조회 정보

1.4.22는 위 운영 마감 상태로 닫고 1.4.23을 시작한다. schema v16과 플랫폼 원격 identity·수집 정책은
변경하지 않는다.

- Google Sheet `도서 목록`은 비어 있던 `장르 후보` 열을 제거하고 고정 열을 A:E로 정리한다.
  열 순서는 `원본 도서명 → 작가 → 장르 → 보유 범위 → 조회 수`다. E열 `조회 수`는 현재 `ok`인
  시리즈 다운로드 수, 카카오 조회 수, 노벨피아 조회 수 가운데 실제로 수집된 값을 합산한다.
  세 수치가 모두 미수집이면 0으로 오해하지 않도록 빈칸을 유지한다.
- 세 플랫폼 상세 열의 기존 개별 수치·장르·태그·링크는 그대로 보존한다.
- Chrome 확장의 Command+우클릭 선택 제목 모달에는 기존 정보 아이콘과 같은 웹 조회수·추천 정보를
  함께 표시한다. EnterJoy·Tcafe21·Pastebin에 더해 Chating Wiki(`chating.wiki`)에서도 활성화한다.
  Chating Wiki의 자체 모달 위에서도 보이도록 확장 모달은 브라우저 top layer dialog로 표시하며,
  자료함 카드와 게시판 행의 제목 옆에도 기존 로컬 중복 돋보기와 웹 정보 아이콘을 붙인다. 동적으로
  추가되는 카드·행도 기존 주기 스캔으로 처리한다. 아이콘 호버 툴팁도 비차단 top layer popover로 표시해 자료함에 가리지
  않게 한다. 자료함 제목의 `+`는 이 사이트가 표시한 공백 구분자로만 해석하며, 공용 normalizer의
...
만듭니다. A열 원본 도서명·B열 작가·C열 장르·D열 보유 범위·E열 조회 수와 상단 두 행을 고정합니다.
비어 있던 장르 후보 열은 표시하지 않습니다. E열은 현재 성공 상태인 시리즈 다운로드 수·카카오 조회 수·
노벨피아 조회 수의 합이며, 실제 수치가 하나도 없으면 빈칸입니다. 그 뒤에 플랫폼별
