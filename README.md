...
자동 교체는 TXT 두 파일에 대해 다음 조건을 **모두** 만족할 때만 실행합니다.

1. 정규화한 `core_title`이 같고 회차 단위가 호환됩니다.
2. 시작 회차가 같으며 긴 파일의 종료 회차가 더 큽니다. `1-100 → 1-150`은 해당하지만,
   `1권 → 2권`, 본편과 별도 외전, 좌표가 불명확한 파일은 해당하지 않습니다.
3. 현재 fingerprint와 파일 identity를 다시 확인하고 다음 본문 증거 중 하나를 충족합니다.
   - 짧은 파일의 정규화 전체 본문이 긴 파일의 동일 길이 접두 본문과 SHA-256까지 일치
   - 배포 머리말 차이 등으로 접두가 달라도, 짧은 본문 전역의 고유 앵커 5개가 긴 본문에서
     같은 순서와 거의 같은 간격으로 정확히 일치
4. 관리 중인 work/variant가 서로 충돌하지 않습니다. 기존 대표를 교체할 때는 긴 파일을 먼저
   house에 journal 입고하고 대표 관계를 넘긴 다음 짧은 파일을 격리합니다.
...
다음 관계만 review/warning에 남깁니다.

- 양쪽에 명시된 작가가 서로 다름
- 시작·종료 회차 또는 단위가 충돌하거나 모호함
...
각 자동 교체는 dedup JSON의 `contained_upgrade_count`와 `status=superseded` 레코드에 짧은/긴
파일, 현재 normalized SHA-256, 본문 길이, 앵커 개수·간격, house ingest operation ID,
quarantine operation ID와 최종 격리 경로를 남깁니다. 이 정책을 수정할 때는 반드시
`public_tests/test_contained_version_upgrade.py`의 정방향·역방향·머리말 차이·작가 충돌 회귀를
함께 변경하고, README에서 자동/수동 경계가 왜 바뀌는지 명시해야 합니다.
