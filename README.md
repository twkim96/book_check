...
- 선두의 `갱신`, `업데이트`, `업뎃`, `재업로드`, `수정본`, `교체`, `추가`는 `19禁`·`19금`·
  `완결` 상태표시와 닫는 구두점이 함께 있거나, 그 상태어 자체가 대괄호/괄호로 닫힌 경우만 제거합니다.
  따라서 `나는 매달 치트키를 갱신할 수 있다`의 `갱신`은 제목으로 보존합니다.
...
Python과 Chrome 확장은 같은 `NORMALIZER_VERSION=1.3.1`과 같은 결과를 사용합니다. 이 버전은
파일명·`core_title` 의미만 바꿉니다. TXT/EPUB 본문 fingerprint와 pair 판정 의미는 바뀌지 않았으므로
